...
public abstract class JNachos {
	...
	public static ArrayList<Integer>  processIdList;
	
	public static int[][] BankerMAX=new int[10][10];
	
	public static int[][] BankerAlloc=new int[10][10];
	
	public static int[][] BankerNeed=new int[10][10];
	
	public static int[] BankerAvai=new int[10];
	...
	public static ArrayList<Semaphore> semObjList;
	

	public static ArrayList<String> resourceStringList;
	...
	public static void addProcess(int x)
	{
		processIdList.add(x);
	}
	...
	public static int add(String sp,int count)
	{
		if(!resourceStringList.contains(sp))
		{
			int index=semObjList.size();
			Semaphore temp = new Semaphore(sp, 0,index);
			semObjList.add(temp);
			resourceStringList.add(sp);
			BankerAvai[index]=count;
			nR++;
			return index;
		}
		return semObjList.indexOf(sp);
	}
	
	public static int numberOfResource()
	{
		return nR;
	}
	
	public static int processIdRemove(int x)
	{
		return processIdList.remove(processIdList.indexOf((Integer)x));
	}
	
	public static int getBankerAvai(int x)
	{
		return BankerAvai[x];
	}
	
	public static void setMax(int pId,int rId,int count)
	{
		BankerMAX[pId][rId]=count;
	}
	
	public static Semaphore getSemObj(int x)
	{
		return semObjList.get(x);
	}
	
	public static void PCalled(int pId,int resourceId)
	{
		BankerAvai[resourceId]--;
		BankerAlloc[pId][resourceId]++;
	}
	
	public static void VCalled(int PId,int resourceId)
	{
